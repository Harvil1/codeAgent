...
**Files:**
- Modify: `HARVIL.md`（在「常用命令」和「关键代码位置」加 handoff）
- Modify: `C:\Users\Administrator\Desktop\HarvilAgent会话交接.md`（更新剩余清单）

- [ ] **Step 1: 更新 HARVIL.md**

在「常用命令」区追加（在 `/handoff` 还没有，先把 CLI 内置命令补全）：
...
```bash
git add HARVIL.md
git commit -m "docs(handoff): HARVIL.md 加 handoff 命令 + 关键代码位置
...
Expected: 推送成功，5 个新 commit（4 个 feat + 1 个 docs）+ 之前的 spec commit
