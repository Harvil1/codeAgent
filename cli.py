    ...
    try:
        ...
        if resumable:
            ...
            parts.append(
                ...
                "——可带着已做部分续跑，不必从 0 重来。恢复姿势：\n"
                "1. **同一条回复里连发全部 subagent_resume 调用**（不要先跑"
                " tail/ls 之类的侦查命令——轨迹都在，直接恢复才是正事；连发"
                "时屏幕会像当初派遣时一样逐个亮起恢复行）；\n"
                "2. 中断期间用户可能亲自改过文件——续跑子代理的旧上下文"
                "可能与磁盘不符，它们会先重读关键文件再继续（写文件另有"
                "指纹校验兜底，外部改过会被拒写逼重读）；\n"
                "3. 不想续跑的（任务已被用户手工完成）可以跳过，在最终"
                "汇报里说明即可。）\n"
                + "\n".join(lines)
                + "\n</subagent_recovery>"
            )
