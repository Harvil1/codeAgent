...
Only proceed to Step 1b if the native tool is unavailable (e.g. the repo is not a git repo, or creation fails and `create_isolated_workspace` degraded to a temp directory).
...
| Native worktree tool available (`create_isolated_workspace`) | Use it (Step 1a) |
...
- Use `git worktree add` when you have a native worktree tool (`create_isolated_workspace`). This is the #1 mistake — if you have it, use it.
