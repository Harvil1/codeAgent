...
- **inline mcpServers（agent .md）走项目级审批** —— 项目来源（.omnimate/agents）的内联 server 与项目 .mcp.json 同款首连审批（启动时问一次 + key 带配置指纹）；未批准 spawn 时 fail-closed 跳过。用户级/CLI 注入的 agent 定义是信任源不拦。
...
- **workflow kill/cancel 是边界语义** —— kill 经 `_ACTIVE_RUNS` 事件只在 agent() 调用间隙生效（to_thread 里的单个子代理不可中断）；前台 run 阻塞主循环，kill 实际只能由另一会话/async 子代理发起。resume 预算是剩余额度制（总额-累计已花，耗尽拒绝 resume）。
