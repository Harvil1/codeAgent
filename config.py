...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "bg_task": {
        ...
        "max_consecutive_wakes": 3,              # 唤醒预算：空闲时最多连续自动唤醒
                                                 # 几次（防"唤醒→起新任务→又完成→
                                                 # 又唤醒"的自激励循环烧 token）；
                                                 # 0=禁用自动唤醒；用户真实输入会
                                                 # 重置计数
    ...
    "plugins": {
        ...
        # 用官方市场的自建镜像（GitHub 主仓，已适配本项目工具名；
        # Gitee 镜像同内容：gitee.com/hong-wei-h/code-agent-plugin-official，
        # 国内拉不动 GitHub 时 settings.json 可换 url）
