...
TOOL_USAGE_GUIDANCE = (
    ...
    "- **代码输出位置**（重要）：\n"
    "  agent 写的代码/脚本/产物统一放在 `~/.agent/` 下(不要写到项目目录):\n"
    "    `~/.agent/workspace/<文件名>` — 脚本、生成物\n"
    "    `~/.agent/workspace/outputs/` — PPT/Excel/Word 等输出文件\n"
    "  然后用 terminal 把产物 `cp`/`move` 到用户指定位置(如桌面)。\n"
    "  如果用户指定桌面等 ~/.agent/ 外的位置,write_file 会触发审批,\n"
    "  用户同意后该目录加入白名单,下次不再询问。\n"
)
