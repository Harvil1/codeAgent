...
- project：**当前工作目录项目**的规则、技术栈决策、业务逻辑。
  ⚠️ 只给当前项目用 project——分析用户指定的**其他路径项目**（外部
  仓库/别的目录）产生的档案（技术栈、结构、部署方式等）一律用
  reference，否则会写错项目分区、污染当前项目的会话上下文。
- **reference**: 外部系统指针——对话中提到的、值得跨会话记住的外部资源位置。
  ...
  也包括：用户本地的**非当前目录**项目档案（如 D:\\project\\xxx 的技术栈
  与路径速查）。提炼时机：用户明确指向某外部资源/项目作为工作对象。
