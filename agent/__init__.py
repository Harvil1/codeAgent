...
# max_tokens 升级/续写恢复四件 + 长退避心跳拆到 llm_retry（拆分二期块 A，
# 行为零变化搬迁）；属性全留 AIAgent，自由函数第一参收 agent 实例（原 self）。
# llm_retry 模块级只 import 标准库（agent.feature_flags 是函数内惰性），
# import 方向 agent/__init__ → llm_retry 无回边，故走模块级引入而非调用点惰性
from agent.llm_retry import (
    llm_retry_heartbeat,
    merge_continuation_response,
    recover_output_truncation,
    try_escalate_max_tokens,
)
# 流式调 LLM 心脏拆到 llm_streaming（拆分二期块 B，行为零变化搬迁）。
# llm_streaming 模块级只 import 标准库 + llm_retry + streaming_executor
# （后两者均不回 import agent root），方向 agent/__init__ → llm_streaming
# 无回边，故走模块级引入而非调用点惰性
from agent.llm_streaming import (
    call_llm_streaming,
    discard_partial_stream_state,
)
...
class AIAgent:
    ...
    def _record_llm_usage(self, response, sent_message_count: int = None) -> None:
        ...
        from agent.usage_accounting import record_llm_usage
        record_llm_usage(self, response, sent_message_count)
    ...
    def _extract_turn_tokens(self, response) -> int:
        """薄委托：本体已拆到 usage_accounting（消费点不改调用面）。"""
        from agent.usage_accounting import extract_turn_tokens
        return extract_turn_tokens(response)

    @staticmethod
    def _extract_cache_read(usage) -> int:
        """薄委托：本体已拆到 usage_accounting（staticmethod 保调用面）。"""
        from agent.usage_accounting import extract_cache_read
        return extract_cache_read(usage)
    ...
    async def run_conversation(
        ...
        while (
            ...
            response = await recover_output_truncation(
                self, response, messages, tool_schemas,
            )
    ...
    async def _call_llm_with_escalation(
        ...
        try:
            if self._stream_callback is not None:
                response = await call_llm_streaming(self,
                    messages=messages,
                ...
                if detect_length_finish(response):
                    new_max = try_escalate_max_tokens(self, "非流式")
