...
def test_all_endpoints_fail_falls_back_to_main():
    main = _FakeClient("main")
    ep1 = _FakeClient("ep1", succeed=False)
...
def test_circuit_skips_endpoint_when_open():
    main = _FakeClient("main")
    ep1 = _FakeClient("ep1", succeed=False)
...
def test_endpoint_without_api_key_skipped(monkeypatch):
...
def test_legacy_aux_config_compatible():
    main = _FakeClient("main")
...
def test_no_config_returns_main_only():
    main = _FakeClient("main")
...
def test_build_router_from_endpoints_config():
