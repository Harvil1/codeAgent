...
Task D4 fix: AuxLLMRouter.chat_completions 改 async 后，测试用 AsyncMock +
async def test_*。
"""
...
async def test_aux_router_no_aux_config_uses_main():
    """aux_config=None 时直接用主 client。"""
    main = MagicMock()
    main.chat_completions = AsyncMock(return_value="main_response")

    router = AuxLLMRouter(main_client=main, main_model="main-model")
    assert not router.is_aux_configured

    result = await router.chat_completions([{"role": "user", "content": "hi"}])
...
async def test_aux_router_uses_aux_when_configured():
    """配置了 aux → 优先用 aux client。"""
    main = MagicMock()
    main.chat_completions = AsyncMock(return_value="main_response")
    aux = MagicMock()
    aux.chat_completions = AsyncMock(return_value="aux_response")
...
async def test_aux_router_fallback_on_aux_failure():
    """aux 抛异常 → 自动 fallback 到主 client。"""
    main = MagicMock()
    main.chat_completions = AsyncMock(return_value="main_response")
    aux = MagicMock()
    aux.chat_completions = AsyncMock(side_effect=RuntimeError("aux API down"))
...
async def test_aux_router_aux_create_failure_uses_main():
...
async def test_aux_router_passes_kwargs():
    """kwargs 应该透传给底层 client。"""
    main = MagicMock()
    main.chat_completions = AsyncMock(return_value="ok")
