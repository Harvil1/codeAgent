...
def test_default_config_has_delegation():
    assert DEFAULT_CONFIG["delegation"]["max_concurrent_children"] == 5
...
def test_apply_profile_default(monkeypatch):
    ...
    monkeypatch.setenv("OMNIMATE_HOME", "/some/path")
    apply_profile("default")
    assert "OMNIMATE_HOME" not in os.environ
...
def test_memory_manager_on_pre_compress_is_noop():
    ...
    with tempfile.TemporaryDirectory() as td:
        store = MemoryStore(omnimate_home=Path(td))
...
def test_default_config_has_sandbox_fields():
    """config 默认值含 sandbox_mode + sandbox_writable_roots。"""
    from config import DEFAULT_CONFIG
    sec = DEFAULT_CONFIG["security"]
    assert sec["sandbox_mode"] == "off"
    assert sec["sandbox_writable_roots"] == []
