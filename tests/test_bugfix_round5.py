...
# ---------------------------------------------------------------------------
# run_interactive 应在 finally 调 rt.shutdown（异常退出也清理）
# ---------------------------------------------------------------------------

def test_run_interactive_shutdown_in_finally():
    """RuntimeContext 用 atexit.register 兜底 shutdown（异常退出也清理）。"""
    ...
    # __init__ 应注册 atexit 兜底（即使主循环异常也会清理）
...
# ---------------------------------------------------------------------------
# agent.cleanup 应关闭 LLM client（HTTP 连接）
# ---------------------------------------------------------------------------

def test_agent_cleanup_closes_llm_clients():
    """agent.cleanup 源码应含 close LLM client 的逻辑。"""
...
# ---------------------------------------------------------------------------
# Cron 一次性 job 触发后应立刻 disable + persist（避免重启重复触发）
# ---------------------------------------------------------------------------

def test_cron_oneshot_disable_before_notification():
    """一次性 job 触发时立刻 disable+persist，再 push 通知。
...
# ---------------------------------------------------------------------------
# macOS Seatbelt profile 用完应清理
# ---------------------------------------------------------------------------

def test_seatbelt_profile_cleanup_after_use():
    """_seatbelt_wrap 或调用方应清理 profile 文件。
...
# ---------------------------------------------------------------------------
# write_file 应使用 atomic_write_text（避免写到一半崩溃）
# ---------------------------------------------------------------------------

def test_write_file_uses_atomic_write():
    """file_operations 源码 write 路径应用 atomic_write_text，不用 path.write_text。"""
...
# ---------------------------------------------------------------------------
# 删除 safe_path 死代码 except
# ---------------------------------------------------------------------------

def test_safe_path_no_dead_except():
    """safe_path 不应有"连续两个 except"死代码模式。

    `except ValueError: continue` 紧跟 `except (OSError, ValueError): continue` 时，
