...
def test_spawn_without_task_id_no_env(tmp_path, monkeypatch):
    """不传 task_id → 子进程 env 不含 OMNIMATE_KANBAN_TASK。"""
    monkeypatch.delenv("OMNIMATE_KANBAN_TASK", raising=False)
    ...
    assert "OMNIMATE_KANBAN_TASK" not in captured["env"]
...
def test_spawn_with_task_id_claims_and_sets_env(tmp_path, monkeypatch):
    ...
    # env 注入
    assert captured["env"]["OMNIMATE_KANBAN_TASK"] == task["id"]
    # claim 持久化
