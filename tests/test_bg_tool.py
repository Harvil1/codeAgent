...
async def test_bg_start_returns_json_with_task_id(tmp_path):
...
async def test_bg_start_full_returns_bg_task_full_error(tmp_path):
    from agent.background import BackgroundManager
    mgr = BackgroundManager(max_concurrent=1)
    await registry.dispatch("bg_start",
...
async def test_bg_status_returns_summary_json(tmp_path):
    from agent.background import BackgroundManager
    mgr = BackgroundManager()
    start_str = await registry.dispatch(
        "bg_start", {"command": _quick_cmd(), "cwd": str(tmp_path)},
        bg_manager=mgr,
    )
    task_id = json.loads(start_str)["task_id"]
    status_str = await registry.dispatch(
...
async def test_bg_status_unknown_returns_not_found():
    from agent.background import BackgroundManager
    mgr = BackgroundManager()
    result_str = await registry.dispatch(
...
async def test_bg_result_returns_full_output(tmp_path):
    from agent.background import BackgroundManager
    mgr = BackgroundManager()
    start_str = await registry.dispatch(
        "bg_start", {"command": _quick_cmd(), "cwd": str(tmp_path)},
...
async def test_bg_list_returns_all_tasks(tmp_path):
    from agent.background import BackgroundManager
    mgr = BackgroundManager()
    await registry.dispatch("bg_start", {"command": _quick_cmd(), "cwd": str(tmp_path)},
                       bg_manager=mgr)
    await registry.dispatch("bg_start", {"command": _quick_cmd(), "cwd": str(tmp_path)},
                       bg_manager=mgr)
    result_str = await registry.dispatch("bg_list", {}, bg_manager=mgr)
    parsed = json.loads(result_str)
    assert len(parsed["tasks"]) == 2
    mgr.shutdown()


async def test_bg_stop_terminates_and_returns_stopped(tmp_path):
    from agent.background import BackgroundManager
    mgr = BackgroundManager()
    start_str = await registry.dispatch(
        "bg_start",
...
async def test_bg_stop_unknown_returns_not_found():
    from agent.background import BackgroundManager
    mgr = BackgroundManager()
    result_str = await registry.dispatch(
...
async def test_bg_start_no_manager_returns_bg_unavailable():
    """bg_manager=None 时返回 bg_unavailable error。"""
    import json
    from tools.registry import registry
    result_str = await registry.dispatch("bg_start", {"command": ["echo", "x"]})
    parsed = json.loads(result_str)
    assert parsed.get("error_type") == "bg_unavailable"


async def test_bg_start_invalid_command_returns_invalid_args(tmp_path):
    ...
    mgr = BackgroundManager()
    # command 缺失
    result_str = await registry.dispatch("bg_start", {}, bg_manager=mgr)
    parsed = json.loads(result_str)
    assert parsed.get("error_type") == "invalid_args"
    # command 非 list
    result_str = await registry.dispatch("bg_start", {"command": "not-a-list"}, bg_manager=mgr)
    parsed = json.loads(result_str)
    assert parsed.get("error_type") == "invalid_args"
    mgr.shutdown()
