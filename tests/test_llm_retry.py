...
def test_backoff_has_jitter(monkeypatch):
    ...
    """
    sleeps = []
    monkeypatch.setattr("agent.llm_retry.time.sleep", lambda s: sleeps.append(s))
    ...
    call_with_retry(llm_client, [], max_retries=3, initial_backoff=1.0)
...
def test_backoff_jitter_within_range(monkeypatch):
    ...
    call_with_retry(llm_client, [], max_retries=2, initial_backoff=1.0)
...
def test_backoff_zero_jitter_falls_back_to_pure_exponential(monkeypatch):
...
def test_retry_after_overrides_backoff_but_keeps_jitter(monkeypatch):
...
# ---------------------------------------------------------------------------
# call_with_retry 透传 max_tokens
# ---------------------------------------------------------------------------

def test_call_with_retry_passes_max_tokens():
    """call_with_retry 把 max_tokens 透传给 chat_completions。"""
    llm_client = _mock_llm_client(_mock_llm_response())
    call_with_retry(llm_client, [], max_tokens=8192)
    # MagicMock 的 call_args_kwargs 拿到 kwargs
    kwargs = llm_client.chat_completions.call_args.kwargs
    assert kwargs.get("max_tokens") == 8192


def test_call_with_retry_without_max_tokens_does_not_pass_it():
    """不传 max_tokens 时，chat_completions 也不会收到 max_tokens=None（避免 SDK 误判）。"""
    llm_client = _mock_llm_client(_mock_llm_response())
    call_with_retry(llm_client, [])
    kwargs = llm_client.chat_completions.call_args.kwargs
    assert "max_tokens" not in kwargs


# ---------------------------------------------------------------------------
# P1-1: 529 连续失败精确切模型（避免浪费重试次数）
# ---------------------------------------------------------------------------

def test_consecutive_529_switches_to_fallback_early(monkeypatch):
...
def test_529_below_threshold_keeps_retrying_main(monkeypatch):
    ...
    assert fallback_client.chat_completions.call_count == 0


def test_529_counter_resets_on_non_529_error(monkeypatch):
    ...
    assert fallback_client.chat_completions.call_count == 0
    assert main_client.chat_completions.call_count == 5


def test_529_threshold_disabled_when_set_to_zero(monkeypatch):
