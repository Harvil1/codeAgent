...
def test_temp_workspace_via_helper():
    ...
    try:
        ...
        cleanup(force=True)  # 智能清理下需 force=True 才能清理有改动的目录
    assert not path.exists()
...
def test_summarize_fallback_on_error():
    ...
    async def fail_create(messages, *, tools=None, **kw):
...
async def test_delegate_with_isolated_workspace():
...
def test_delegate_summary_only_in_schema():
    ...
    delegate = [t for t in tools if t["function"]["name"] == "subagent"][0]
