...
def test_offload_path_under_agent_home(tmp_path: Path):
    ...
    content = "x" * 50000
    result = maybe_offload(
        content, tool_call_id="call_xyz", agent_home=tmp_path,
        threshold=30000,  # 显式传（不依赖 DEFAULT_THRESHOLD 默认值）
    )
    parsed = json.loads(result)
    offload_path = Path(parsed["full_at"])
...
def test_duplicate_tool_call_id_appends_counter(tmp_path: Path):
    ...
    content1 = "x" * 50000
    maybe_offload(
        content1, tool_call_id="call_dup", agent_home=tmp_path, threshold=30000,
    )
    content2 = "y" * 50000
    result = maybe_offload(
        content2, tool_call_id="call_dup", agent_home=tmp_path, threshold=30000,
    )
    parsed = json.loads(result)
...
def test_disk_full_falls_back_to_truncated_content(tmp_path: Path, monkeypatch):
    ...
    content = "x" * 50000
    result = maybe_offload(
        content, tool_call_id="call_err", agent_home=tmp_path, threshold=30000,
    )
    parsed = json.loads(result)
