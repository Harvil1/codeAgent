...
# 触发 session_search 工具注册（registry.register 在 import 时执行）
...
def test_fts5_available():
    """JSONL 版本不再依赖 FTS5（is_fts5_available 返回 False）。"""
    assert is_fts5_available() is False, "JSONL 版本不应需要 FTS5"
...
def test_tool_message_roundtrip(store):
    """assistant(tool_calls) + tool 结果存取往返，恢复时能还原工具轮次。"""
    sid = store.create_session(model="deepseek", provider="deepseek")
    ...
    # user → assistant(tool_calls) → tool 结果
    store.append_message(sid, "user", "读一下 x.py")
    store.append_message(sid, "assistant", "", tool_calls=tool_calls)
    store.append_message(sid, "tool", '{"success": true}', tool_call_id="call_abc123", name="read_file")
    ...
    assert tool["content"] == '{"success": true}'


def test_tool_message_fork_keeps_name(store):
    """fork 会话复制时保留 tool 消息的 name 字段。"""
    sid = store.create_session()
    store.append_message(sid, "user", "hi")
    store.append_message(sid, "assistant", "", tool_calls=[{
        "id": "call_1", "type": "function",
        "function": {"name": "terminal", "arguments": "{}"},
    }])
    store.append_message(sid, "tool", "ok", tool_call_id="call_1", name="terminal")
