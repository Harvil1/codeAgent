...
def test_get_stats_malformed_tool_calls_skipped(tmp_path):
    ...
    with s._get_conn() as conn:
        conn.execute(
            ...
            "tool_call_id, timestamp, turn_index) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            ("m1", sid, "assistant", "", "not-valid-json",
             None, "2026-01-01T00:00:00Z", 1),
        )
    # 不应抛
    stats = s.get_stats()
...
def test_get_stats_malformed_tool_calls_structure_skipped(tmp_path):
    """tool_calls 是合法 JSON 但结构不对（不是 list of dicts）也应跳过。"""
    s = SessionStore(tmp_path / "test.db")
    sid = s.create_session()
    with s._get_conn() as conn:
        conn.execute(
            "INSERT INTO messages (id, session_id, role, content, tool_calls, "
            "tool_call_id, timestamp, turn_index) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            ("m1", sid, "assistant", "", '{"weird": "object"}',
             None, "2026-01-01T00:00:00Z", 1),
        )
        conn.execute(
            "INSERT INTO messages (id, session_id, role, content, tool_calls, "
            "tool_call_id, timestamp, turn_index) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            ("m2", sid, "assistant", "", '["not-a-dict"]',
             None, "2026-01-01T00:00:00Z", 2),
        )
    stats = s.get_stats()
