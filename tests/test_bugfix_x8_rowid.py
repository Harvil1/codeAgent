"""X8 回归 bug 修复测试：get_messages(limit) 报 no such column: rowid。

bug：X8 fix 把子查询改成 ORDER BY rowid DESC LIMIT，但内层 SELECT
...
def test_get_messages_with_limit_does_not_error_on_rowid(tmp_path):
    ...
    # 调 get_messages(limit=2) —— 之前这里抛 sqlite3.OperationalError
